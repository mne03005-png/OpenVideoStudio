...
"An open pipeline that turns a prompt into a full edited video, running
locally on a 6GB GPU — looking for contributors on the parts I
...
prompt → full edited video, open source, looking for contributors on the
parts I left unbuilt on purpose. {REPO_URL}"
...
**Video title:** "I Built an Open-Source AI Video Studio (Runs Locally on
a 6GB GPU)"
...
> project is built and tested on. It's open source (Apache-2.0) and
> actively looking for contributors — timestamps and links below.
...
**Short version:** "从一句提示词到成片，全部由这个我今天开源的软件在一台
笔记本显卡上生成完成。"
...
> 生成视频片段 → Edge TTS 配音 → 字幕 → FFmpeg 自动剪辑合成。全部在本地
> 跑，6GB 显存笔记本卡（RTX 3060）验证通过，具体哪些硬件配置测试过、哪些
> 只是预期可行，`docs/HARDWARE.md` 里如实写清楚了，没有夸大的硬件宣传。
> Provider 层（LLM/图像/视频/语音）是可替换接口，配置驱动，不是写死在
> pipeline 代码里——这点有测试验证，不是嘴上说说。
...
locally" (Edge TTS is disclosed everywhere it would otherwise be
implied), no unqualified "open source" ahead of the Apache-2.0 license
actually being in place (it now is — see `docs/LICENSE_STRATEGY.md`), and
no hardware claim beyond what `docs/HARDWARE.md` actually verifies.
